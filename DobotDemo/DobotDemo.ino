...
float startPosX = 0;       float startPosY = 0;       float startPosZ = 0;       float startPosR = 0;       //Start position when powering up
float currentX = startPosX;     float currentY = startPosY;   float currentZ = startPosZ;   float currentR = startPosR;
/*********************************************************************************************************
** Function name:       setup
...
void loop()
{
  ...
  while(true) {
     moveArm(startPosX, startPosY, startPosZ, startPosR, false);
     delay(2000);
     moveArm(200, 100, 50, 50, true);
     delay(2000);
     moveArm(200, 200, -50, 50, true);
     delay(2000);
  }
}
...
void moveArm(float x, float y, float z, float r, bool vacuumOn)
{
  ...
  SetPTPCmd(&gPTPCmd, true, &gQueuedCmdIndex);
  SetEndEffectorGripper(vacuumOn, true, &gQueuedCmdIndex); // open gripper (compressed air on);
  ProtocolProcess();
